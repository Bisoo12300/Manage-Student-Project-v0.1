...
namespace QLSV
{
    public partial class HRForgotPassword : Form
    {
        Random random = new Random();
        int otp;
        public HRForgotPassword()
        {
            InitializeComponent();
        }
        bool Test()
        ...
        private void button_Next_Click(object sender, EventArgs e)
        {
            if (Test())
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SELECT * FROM HRLogin WHERE Gmail = @gmail ", db.getConnection);
                    ...
                    if (table.Rows.Count > 0)
                    {
                        ...
                        ChangePassHR cp = new ChangePassHR(selectedEmail);
                        cp.Show(this);
