...
namespace QLSV
{
    public partial class ManageUser : Form
    {
        ...
        private void btInsert_Click(object sender, EventArgs e)
        {
            ...
            if (verif())
            {
                try
                {
                    ...
                    SqlCommand command =
                        new SqlCommand("INSERT INTO Login(username,password,gmail,state) VALUES (@user,@pass,@gmail,state)",
                            myDb.getConnection);
        ...
        {
            SqlCommand command =
                new SqlCommand("SELECT * FROM Login WHERE CONCAT(username,gmail) LIKE '%" + txtSearch.Text + "%'");
            fillGrid(command);
        ...
        void fillGrid(SqlCommand command)
        {
            ...
            dataGridView1.RowTemplate.Height = 80;
            dataGridView1.AllowUserToAddRows = false;
            dataGridView1.DataSource = command.CommandText = "SELECT * FROM Login";
            ;
        ...
        private void btEdit_Click(object sender, EventArgs e)
        {
            ...
            if (verif())
            {
                ...
                if (user != "")
                {
                    SqlCommand command = new SqlCommand("UPDATE Login SET password=@pass WHERE username=@user",
                        myDb.getConnection);
                    command.Parameters.Add("@user", SqlDbType.VarChar).Value = user;
                    command.Parameters.Add("@pass", SqlDbType.VarChar).Value = pass;
                    myDb.openConnection();
        ...
        private void buttonBlock_Click(object sender, EventArgs e)
        {
            ...
            if (verif())
            {
                ...
                if (user != "")
                {
                    SqlCommand command = new SqlCommand("UPDATE Login SET state=@state WHERE username=@user",
                        myDb.getConnection);
                    command.Parameters.Add("@user", SqlDbType.VarChar).Value = user;
                    command.Parameters.Add("@state", SqlDbType.Bit).Value = state;
                    myDb.openConnection();
